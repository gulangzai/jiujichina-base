package com.lanbao.controller.system.head;
...
import com.lanbao.service.system.appuser.AppuserService;
import com.lanbao.service.system.user.UserService;
import com.lanbao.utils.AppUtil;
import com.lanbao.utils.Const;
import com.lanbao.utils.SmsUtil;
import com.lanbao.utils.Tools;
import com.lanbao.utils.Watermark;
import com.lanbao.utils.mail.SimpleMailSender; 
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",lanbao,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",lanbao,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",lanbao,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",lanbao,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",lanbao,");
			if(strIW.length == 4){
		...
		if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
			String strIW[] = strWEBSOCKET.split(",lanbao,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",lanbao,"+pd.getString("PORT")+",lanbao,"+pd.getString("EMAIL")+",lanbao,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",lanbao,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",lanbao,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",lanbao,"+pd.getString("fcontent")+",lanbao,"+pd.getString("fontSize")+",lanbao,"+pd.getString("fontX")+",lanbao,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",lanbao,"+pd.getString("imgUrl")+",lanbao,"+pd.getString("imgX")+",lanbao,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",lanbao,"+pd.getString("WIMPORT")+",lanbao,"+pd.getString("OLIP")+",lanbao,"+pd.getString("OLPORT"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
