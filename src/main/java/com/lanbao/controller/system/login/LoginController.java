package com.lanbao.controller.system.login;
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				List<PageData> list = menuService.listAll(pd);
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if(pd1.get("F_MENU_PARENT")==null){
						...
						tm.setF_menu_serial(Integer.parseInt(pd1.get("F_MENU_SERIAL").toString()));
						allmenuList.add(tm);
					}
				}
				
			  //Collections.sort(allmenuList);  
			  
				for (int i = 0; i < allmenuList.size(); i++) {
					  Menu tm = allmenuList.get(i);
					  for (int j = 0; j < list.size(); j++) {
						  PageData pd1 = list.get(j);
							if(tm.getF_menu_id().equals(String.valueOf(pd1.get("F_MENU_PARENT")))){
								...
								tm1.setF_menu_serial(Integer.parseInt(pd1.get("F_MENU_SERIAL").toString()));
								tm.getTbMenus().add(tm1);
							}
					  } 
					  //Collections.sort(tm.getTbMenus());   
				}
